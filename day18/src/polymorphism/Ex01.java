...
public class Ex01 {
	public static void main(String[] args) {
		...
		// 상속관계는 카테고리(범주)상 포함관계일 때 사용할 수 있으면
		// A is B 관계가 성립해야 한다
		// 강아지(Dog)는 동물(Animal)이다
		// 고양이(Cat)는 동물(Animal)이다
...
//		a1.play(); 	// Animal 클래스에서는 play() 메서드가 없다(Cat의 고유기능)
//		a2.tailing(); 	// Animal 클래스에서는 tailing() 메서드가 없다(Dog의 고유기능)
		
		
		a1.bark();	// 슈퍼클래스 Animal에는 name + " : ... " 으로 정의되어 있으나
		a2.bark();	// 각 서브클래스의 오버라이딩 내용으로 실행된다. ★★★★★
		
		// upcasting : 서브클래스의 객체를 슈퍼클래스로 참조하는 형태 (자료형 반환)
		// 공통된 자료를 묶어서 일괄처리 장점
		
//		Animal[] arr = { a1, a2 };		// 배열에 담긴다.
		Animal[] arr = { ob1, ob2 };	// 자료형이 바껴도 Animal 배열에 담긴다
...
//			arr[i].play();		// Cat의 고유메서드인데 Animal을 참조하기 때문에 안됨
//			arr[i].tailing();	// Dog의 고유메서드인데 Animal을 참조하기 때문에 안됨
