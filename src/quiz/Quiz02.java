...
*/
class Customer{
		// 1) 문자열 형태의 이름과 정수형태의 나이를 저장할 수 있는 필드를 작성하세요
		String name;
		int age;
		...
		// 이때, 값을 초기화하지 않은 age값은 0이다. primitive 형식 (null이 아님)
...
public class Quiz02 {
	public static void main(String[] args) {
		...
		Customer ob1 = new Customer("홍진호", 41);
		Customer ob2 = new Customer("아이유", 30);
		Customer ob3 = new Customer("홍길동", 17);
		...
		for(int i = 0; i < arr.length; i++) {
			Customer member = arr[i];						// 배열 안의 각 손님 객체를 member라고 할때
