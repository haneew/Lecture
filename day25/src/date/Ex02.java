...
public class Ex02 {
	public static void main(String[] args) throws Exception {
		...
		// java.sql.Date는 java.uil.Date의 서브클래스이다
		...
		// java.text.SimpleDateFormat 을 이용해서 문자열 <-> 날짜 형식을 활용할 수도 있다.
		// y : year, 연도
		// M : Month, 월
		// d : date, 일
		// h : hour, 시간 		H : 24시간제 시간
		// m : minutes, 분
		// s : seconds, 초
		// a : am/pm, 오전/오후
		SimpleDateFormat sdf = new SimpleDateFormat("yyyy-MM-dd");
		java.util.Date d4 = sdf.parse("1993-05-16");		// parse : 문자열 -> util.Date
		...
		String s1 = sdf2.format(d5);		// format : date를 String으로 
		...
		String s3 = sdf3.format(d5);							// 24시간제로 만든 d5 (s3)
		d4.setTime(d4.getTime() + (1000 * 60 * 60 * 14));		// d4에서 14시간을 더한뒤 문자열로 출력 (s4)
