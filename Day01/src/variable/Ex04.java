...
public class Ex04 {
	public static void main(String[] args) {
		...
		short sh = 32767;
		char ch = 44032;
		long ln = 300000000L;	// 기본자료형이 아니기 떄문에 long임을 명시한다.
		float f1 = 1.2F;		// 기본자료형이 아니기 때문에 float임을 명시한다.
		double db = 3.14;
		
		// The value of the local variable bo is not used
		// 지역 변수 bo는 아직 한번도 사용되지 않았습니다.
		// 변수는 나중에 값을 다시 사용하기 위해서 만드는건데, 왜 한번도 안썻지? 라고 이클립스가 되물음
		// 완성된 코드에는 노란색이 없다. 실행은 되겠지만 불필요한 코드가 있다는 소리?
...
//		int num = "점심식사"; 정수형인데 문자형을 담았으므로 맞지 않는다.
		
//		age = "배고파";
		//이미 선언된 변수는 자료형이 나타나지 않지만, 튤팁이나 ctrl + 클릭 하면 변수 선언으로 이동한다.
		
		// 양쪽다 변수를 나타내고 있지만
		// 오른쪽의 변수는 변수에 담긴 값을 의미하고
		// 왼쪽의 변수는 변수공간 그 자체를 의미한다.
		
//		sh = ch;  //두자료형의 범위와 표현 방법이 다르기 떄문에 맞지 않는다.
		//자료형을 강제로 맞추면 대입이 된다
