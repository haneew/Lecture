...
public class Ex06 {
	...
	// 이항 연산자 : 2개의 항에 대한 연산을 수행 (+ * 단 두개의 항만 해당하는게 아니다. 여러 항이 합쳐 질 수 있다. 헷갈림
	// 상항 연산자 : 3개의 항에 대한 연산을 수행 (+)
	public static void main(String[] args) {
		// 논리 연산자
		// and (&&) : 서로 다른 두 조건이 모두 참이면 결과가 참, 아니면 거짓
		// or (||) : 서로 다른 두 조건 중 하나라도 참이면 참, 아니면 거짓
		// not (!) : 하나의 조건이 참이면 거짓, 거짓이면 참
		
		int age = 25;
		int gender = 1;				// 홍수는 남성, 짝수는 여성
...
//		System.out.println("s2의 길이 : " + s2.length());
		...
		for(int i = 1; i < 20; i++) {						// 1에서 10 사이의 정수 중에서
