...
public class Ex01 {		
	public static void main(String[] args) {
		...
		// java.util.List는 인터페이스라서 객체를 생성 할 수 없다
		...
		// list.add(element) 함수는 리스트의 가장 마지막에 요소를 추가한다.
		list.add(10);
		list.add(20);
		
		// 리스트 및 컬렉션은 대부분 toString()이 오버라이딩 되어있어서 바로 출력해도 내용을 확인 할 수 있다.
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
		
		// list.add(int index, E e) 함수는 지정한 인덱스에 값을 추가한다
		list.add(1, 30);							// 1번째 자리에 30을 추가한다. 기존의 값은 뒤로 밀려난다.
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
		
		// 특정 인덱스의 값을 제거하거나, 값을 지정하여 제거할 수 있다.
		list.remove(0);								// 0번째 자리의 값을 제거한다.
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
		
//		list.remove(30);							// 정수를 그냥 전달하면 index로 간주한다.
		list.remove(Integer.valueOf(30));			// Integer 타입 객체를 전달하면 일치하는 값을 찾아서 제거한다.
		System.out.println("list : " + list);
		System.out.println("list.size() : " + list.size());
		...
		System.out.println(new int[] {20, 40, 10, 50, 30});   		// 기존 배열은  toStringArrays로 변환해야했다.
		System.out.println(list);									// 리스트는 기본 내장되어 있음
