...
class Test4{
	...
	static int n2 = 20;			// 정적 멤버 필드, 모든 객체가 값을 [공유]하고 변경할 수 있다.
	final int n3 = 30;			// 불변 멤버 필드, 객체마다 다른 값을 가지고 변경할 수 없다
	static final int N4 = 40;	// 정적 불면 멤버 필드, 모든 객체가 값을 공유하고 변경 할 수 없다.
	
	// 클래스를 제외한 모든 요소는 첫글자를 소문자로 작성하지만
	// static final 필드는 예외적으로 모든 글자를 대문자로 작성한다.
	// 단어와 단어의 합성인 경우는 각 단어를 구분하기 위한 방법으로 _(언더바)를 사용한다.	(참조)
	
	public static int getN2() {		// static 메서드는 static 필드를 참조해야 한다.
		return n2;					// non-static 필드는 참조할 수 없다. (여기서 return n1;)
	}
}
...
public class Ex04 {
	public static void main(String[] args) {
		...
		System.out.println(Integer.MAX_VALUE);		// 참조
