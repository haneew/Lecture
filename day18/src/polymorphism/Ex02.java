...
class Human {
	...
	void info() {
		...
		// 서브클래스에서는 이 메서드를 오버라이딩해서 이름과 직업을 출력해야 합니다.
	}
}
...
class Airplane { 
	...
	public void simulationEmergency(int index) {
		...
		if(tmp == null) {
			...
			System.out.println("자기자신을 치료할 수 없습니다");
		}
		else {
			tmp.cure(seat[index]);
		}
...
/*		Human[] queue = { doctor, guset1, guest2, guest3, guest4 };		// 배열(대기열)
		...
		Human test = guset1; // 명시적 형변환 없이, 암묵적으로 형변환이 이루어진다.
		...
		// 오버라이딩이 아닌 고유 메서드는 다운캐스팅 후 실행해야 된다.
		// 다운클래 하기 전에 instanceof를 활용하여 타입을 확인하고 형변환을 수행해야 한다.
	}
