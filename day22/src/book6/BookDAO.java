...
//--------------------------------------------------------------------------------------------
	...
	public ArrayList<BookDTO> selectList() {
		...
		try {
			...
			rs = pstmt.executeQuery();					// 실행해서 결과를 rs에 저장(즉 developer에서 컨트롤 엔터를 눌렀다고 생각)
			while(rs.next()) {		// 불러온 결과를 한줄씩(rs.next) 처리하기 위해 반복문(다음줄이 있으면 실행)
			...
			System.out.println("selectList 예외발생 : " + e);
			e.printStackTrace();
		} finally {
			try { 
...
//		사용하는 형식은 printf()와 유사하다
		...
		try {
			...
			System.out.println("insert Book 예외 발생 : " + e);
			e.printStackTrace();
		} finally {
			try { 
				if(rs != null) rs.close();		// 통로 개설의 역순으로 닫아준다
