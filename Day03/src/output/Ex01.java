...
public class Ex01 {
	public static void main(String[] args) {
		...
		Date today = new Date();	// 실행하는 시점의 현재 날자 및 시간이 저장된 객체
		System.out.println(today);
		System.out.println(today.toString()); // 모든객체는 String 문자열로 출력가능하다 Ex02의  %s
		
//		System.out.println은  , 를 쓰지 못한다
		...
		System.out.print("A");		// print() 는 출력후 줄을 바꾸지 않는다
		...
		// 문자열이 양쪽 중 한곳에 포함되면, 문자열을 베이스로 하여 글자를 이어붙이기 한다.(append)
		...
		for(int i = 0; i < 5; i++) {
			System.out.print(i + " ");	 // i 를 출력하고 한칸 띄어쓰기하고 줄은 바꾸지 않기.
		}
