...
//		int[] arr2 = new int[]{ 1, 2, 3, 4, 5 };
		...
		arr = new int[] { 1, 3, 5, 7, 9 };	// 이렇게 new int[] 를 이용하면 가능하다.
		
		// 배열에서는 index가 존재하고, index는 항상 0부터 시작한다
		/*
		  		index [0][1][2][3][4]
		  		arr = { 1, 3, 5, 7, 9 }
		...
		for(int i = 0; i < 5; i++) {			//  10 20 30 40 50이 나오게 만들고 싶다
//			System.out.println((i + 1) * 10);
		...
		for(int i = 0; i < arr.length; i++) {
			if(arr[i] % 3 == 0) {						// arr[i]를 단일 변수처럼 생각해라. 연산/출력 가능.
				sum += arr[i];
			}
		}
