...
abstract class Loop {		// 추상클래스는 단일 상속만 가능하다
...
//		ob2.start();	// 다중 실행 요소를 먼저 호출해두면 함수가 종료되지 않아도 다음 함수를 호출한다.
//		ob1.loop();		// ob1에서도 다중 실행이 가능한 특성을 추가도 부여하고 싶다
						// Thread 클래스를 상속받으면 된다
						// NumberLoop는 이미 Loop라는 슈퍼클래스가 있다
						// 자바에서는 클래스의 다중상속을 허용하지 않는다
		
//		1 A B C 2 D 3 E F G H I 4 J 5 K 6 L 7 M 8 N 9 O 10 P 
//		Q R S T U V W X Y Z 
